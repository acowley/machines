...
module Data.Machine.Concurrent.Buffer where
...
-- | Eagerly request upstream values into a buffer that is drained by
-- the wrapped machine. An upstream source will be blocked if the
-- buffer fills up.
buffer :: MonadBaseControl IO m => Int -> MachineT m k o -> MachineT m k o
buffer n src = bufferConnect n src echo

-- | Eagerly request upstream values into a buffer that is drained by
-- the wrapped machine. Values yielded by upstream will be dropped if
-- the wrapped machine can not keep up.
